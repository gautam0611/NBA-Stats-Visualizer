...
def get_all_teams(db: Session, conference_id: int):
    return (
        db.query(models.Team).filter(models.Team.conference_id == conference_id).all()
    )
...
def get_team(db: Session, team_id: int, conference_id: int):
    return (
        ...
        .filter(
            and_(models.Team.id == team_id, models.Team.conference_id == conference_id)
        )
        .first()
    )
...
def get_games(db: Session, season_id: int, team_id: int):
    return (
        ...
        .filter(and_(models.Season.id == season_id, models.Season.team_id == team_id))
        .all()
    )
...
def get_players(db: Session, team_id: int, season_id: int):
    return (
        ...
        .join(models.Roster, models.Player.roster_id == models.Roster.id)
        .join(models.Season, models.Roster.season_id == models.Season.id)
